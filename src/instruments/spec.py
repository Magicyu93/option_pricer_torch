...
These are plain frozen dataclasses, deliberately free of QuantLib types: a spec
is something you can build from a JSON request, put in a database, hash into a
cache key, and diff in a test. The translation to QuantLib objects happens once,
in :mod:`qldesk.instruments.build`.

Quantities and multipliers live on the *position* (:mod:`qldesk.risk.portfolio`),
not on the instrument. A spec describes one contract.
"""
...
class VanillaOption(Instrument):
    ...
    strike: float = 0.0
    maturity: dt.date = dt.date.today()

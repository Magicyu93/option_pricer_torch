    ...
    if isinstance(spec, VanillaOption):
        if spec.style is not Style.EUROPEAN:
            raise ValidationError(
                f"{spec.style.value} exercise is not supported by the Monte Carlo "
                "engine; it prices the terminal payoff only. Pricing an early-"
                "exercisable contract as European would understate it silently."
            )
